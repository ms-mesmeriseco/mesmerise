...
export default function ListCard({ icon, children, outline, ...props }) {
  ...
  return (
    <motion.div
      ...
      className={`flex md:flex-row flex-col gap-6 items-start bg-[var(--mesm-grey-xd)] border-1 border-[var(--mesm-grey-dk)] md:py-4 py-8 px-8 rounded-md shadow ${boxStyle}`}
      {...props}
    >
      {icon?.url && (
        <img
